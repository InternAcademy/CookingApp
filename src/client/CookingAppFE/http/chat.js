...
export async function continueChat({ token, chatId, messageType, content, image }) {
  ...
  const response = await fetch(`${ip}:8001/continue`, {
    method: "POST",
    headers: headers,
    body: body
  });
...
export async function newChat({ token, messageType, content, image }) {
  ...
  const response = await fetch(`${ip}:8001/new-chat`, {
    method: "POST",
    headers: headers,
    body: body
  });
...
export async function getChat({ token, chatId }) {
  const response = await fetch(`${ip}:8001/c/${chatId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
...
export async function getUserChats({ token, userId }) {
  const response = await fetch(`${ip}:8001/user-chats/${userId}`, {
    headers: {
      Authorization: `Bearer ${token}`,
      "Content-Type": "application/json"
    }
