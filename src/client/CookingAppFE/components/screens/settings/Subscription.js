    ...
    <ScrollView style={tw`flex-1 ${isDarkTheme ? "bg-[#202020]" : "bg-white"}  `}>
      <View style={tw`flex-1 items-center p-6 mt-10`}>
        ...
        <View style={tw`flex-row w-full px-6 mb-10 justify-between`}>
          {isPending && (
            <View style={tw`mb-4`}>
              <Spinner size="large" color={isDarkTheme ? "#ffffff" : "#000000"} />
            </View>
          )}
          {isError && (
            <View style={tw`mb-4`}>
              <Text style={tw`text-xs mb-1 ${isDarkTheme ? "text-white" : "text-black"}`}>{error.message}</Text>
            </View>
          )}
          {data && (
            <>
              ...
              {/* Subscriptions */}
              {data.data.map(sub => (
                <View key={sub.id} style={[tw`flex-1 p-4 rounded-lg items-center`, { backgroundColor: isDarkTheme ? "#303030" : "#FFF4E2" }]}>
                  <Text style={tw`text-lg font-semibold mb-2 ${isDarkTheme ? "text-white" : "text-black"}`}>{sub.name}</Text>
