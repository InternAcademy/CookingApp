...
const Home = () => {
  ...
  const renderPost = () => {
    if (chat) {
      ...
      const botIcon = isDarkTheme
        ? require("../../assets/Main/icon2_dark.png")
          ...
          <ScrollView contentContainerStyle={tw`p-6 mt-10`}>
            {chat.content.map((msg, index) => (
              <View key={index} style={tw`mb-2 flex-row justify-start wrap pt-1`}>
                <Image source={msg.role === "user" ? require("../../assets/NavigationBar/user.png") : botIcon} style={tw`w-8 h-8 rounded-full mr-2 mb-7 items-start -mt-1`} />
                <View>
                  ...
                  {msg.role === "user" && msg.type === "Image" && <Image source={{ uri: msg.content }} style={tw`w-32 h-32 rounded-full mr-2 mb-7`}></Image>}
    ...
    const initialIcon = isDarkTheme
      ? require("../../assets/Main/icon2_dark.png")
      : require("../../assets/Main/icon2.png");

    return (
      <View style={tw`flex w-full h-full justify-center items-center`}>
        <Image source={initialIcon} style={tw`w-26 h-26 mb-2`} />
